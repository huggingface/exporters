...
class Wrapper(torch.nn.Module):
    ...
    def forward(self, inputs, bool_masked_pos=None):
        ...
        if isinstance(self.model, BeitForImageClassification):
            return nn.functional.softmax(outputs[0], dim=1)

        if isinstance(self.model, BeitForSemanticSegmentation):
            x = outputs[0]
            if self.do_upsample:
        ...
        if isinstance(self.model, BeitModel):
...
def export(
    ...
    **kwargs,
    ...
    if not isinstance(feature_extractor, BeitFeatureExtractor):
    ...
    if isinstance(torch_model, BeitForMaskedImageModeling):
    ...
    if isinstance(torch_model, BeitForImageClassification):
    ...
    if isinstance(torch_model, BeitForSemanticSegmentation):
    ...
    if isinstance(torch_model, BeitModel):
        # Rename the outputs.
